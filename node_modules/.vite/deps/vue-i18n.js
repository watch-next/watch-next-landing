...
function getPluralIndex(options) {
  ...
  return isNumber((_a = options.named) == null ? void 0 : _a.count) ? options.named.count : isNumber((_b = options.named) == null ? void 0 : _b.n) ? options.named.n : index;
}
function createMessageContext(options = {}) {
  var _a, _b, _c, _d;
  const locale = options.locale;
  const pluralIndex = getPluralIndex(options);
  const pluralRule = isString(locale) && isFunction((_a = options.pluralRules) == null ? void 0 : _a[locale]) ? options.pluralRules[locale] : pluralDefault;
  ...
  const normalize = isFunction((_b = options.processor) == null ? void 0 : _b.normalize) ? options.processor.normalize : DEFAULT_NORMALIZE;
  const interpolate = isFunction((_c = options.processor) == null ? void 0 : _c.interpolate) ? options.processor.interpolate : DEFAULT_INTERPOLATE;
  const type = isString((_d = options.processor) == null ? void 0 : _d.type) ? options.processor.type : DEFAULT_MESSAGE_DATA_TYPE;
...
function defineMixin(vuei18n, composer, i18n) {
  return {
    ...
    unmounted() {
      ...
      delete this.$n;
      delete this.$tm;
      if (_vueI18n == null ? void 0 : _vueI18n.__disposer) {
        _vueI18n.__disposer();
